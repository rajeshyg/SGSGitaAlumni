...
### Preferred Option: D (4-Phase Lifecycle Model)

**Rationale**:
1. **Complete Invitation Coverage**: Phase 1 covers entire invitation lifecycle (admin + user)
2. **No Overlap**: Clear boundaries between phases
3. **Code Alignment**: Matches actual code module organization
4. **Implementation-Ready**: Aligns with implementation-context-family-onboarding-coppa.md scope
5. **COPPA Isolation**: Family onboarding + COPPA in single phase (Phase 2)
6. **Clear Cut-off Points**:
   - Phase 1 ends: Invitation accepted
   - Phase 2 ends: FAMILY_MEMBERS records created
   - Phase 3 ends: User logged in with profile selected
   - Phase 4: Everything else

### Implementation Steps

1. **Create Phase 1: Invitation & Access Control** (3 new specs + 1 existing)
   - invitation-generation.md (new - admin perspective)
   - invitation-acceptance.md (new - user perspective)
   - invitation-tracking.md (new - audit/analytics)
   - invitation-expiry.md (new - lifecycle management)

2. **Create Phase 2: Registration & Family Onboarding** (6 specs)
   - registration.md (expand existing)
   - family-discovery.md (new)
   - relationship-collection.md (new)
   - age-verification.md (move + expand from authentication/)
   - parental-consent.md (move + expand from authentication/)
   - profile-creation.md (new - FAMILY_MEMBERS insertion)

3. **Create Phase 3: Authentication & Identity Management** (6 specs)
   - login.md (move from authentication/)
   - profile-selection.md (new)
   - session-management.md (move from authentication/)
   - profile-switching.md (extract from family-member-management.md)
   - password-management.md (new)
   - account-settings.md (move from user-management/)

4. **Reorganize Phase 4: Platform Features**
   - Move profile-editing.md from user-management/
   - Move add-family-members.md (post-registration additions)
   - Keep existing feature modules (directory, postings, messaging, etc.)
   - Move admin/ features here (excluding invitations)
...
│   └── db-schema.md
│
├── phase-4-features/             # NEW (renamed from existing modules)
│   ├── README.md
│   ├── profile-management/
│   ├── directory/
│   ├── postings/
...
│   └── admin/
│
└── [OLD STRUCTURE KEPT TEMPORARILY FOR REFERENCE]
```
...
Add to old files:
```markdown
> **DEPRECATED**: This spec has moved to `phase-X-name/new-file.md`
